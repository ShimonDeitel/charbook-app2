import XCTest
@testable import Charbook

@MainActor
final class CharbookTests: XCTestCase {
    var store: Store!
    ...
    func testAddIncreasesCount() {
        ...
        store.add(Burn(title: "Test Entry"))
        XCTAssertEqual(store.items.count, before + 1)
    }

    func testDeleteDecreasesCount() {
        store.add(Burn(title: "To Delete"))
        let before = store.items.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        store.isPro = false
        store.items = (0..<Store.freeLimit).map { _ in Burn(title: "x") }
    ...
    func testPersistenceRoundTrip() {
        store.add(Burn(title: "Persisted"))
        store.save()
        let reloaded = Store()
